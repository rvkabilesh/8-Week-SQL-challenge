...
select * from regions;
select region_name,count(distinct node_id) from customer_nodes join regions using(region_id)
group by region_name
order by region_name;
-- How many customers are allocated to each region?
select region_name,count(distinct customer_id) from customer_nodes join regions using(region_id)
group by region_name
order by region_name;
...
with cte as (select region_id,avg(datediff(end_date,start_date)) reallocation_days from customer_nodes
where year(end_date) <> 9999 group by region_id)

select region_id,
PERCENT_rank() OVER(PARTITION BY region_id order by reallocation_days)*100
from cte;*/

-- B. Customer Transactions
-- What is the unique count and total amount for each transaction type?
select distinct txn_type,count(txn_type),sum(txn_amount) from customer_transactions
group by txn_type;
-- What is the average total historical deposit counts and amounts for all customers?
    ...
    select txn_type,avg(deposit_count),avg(deposit_amount) from cte
    where txn_type = 'deposit'
    group by txn_type;
-- For each month - how many Data Bank customers make more than 1 deposit and either 1 purchase or 1 withdrawal in a single month?
with cte as (
SELECT 
    customer_id,monthname(txn_date) mn,
        count(CASE WHEN txn_type = 'deposit' THEN 1 end) deposit,
        count(CASE WHEN txn_type = 'withdrawal' THEN 1 end) withdrawal,
        count(CASE WHEN txn_type = 'purchase' THEN 1 end) purchase
FROM
    customer_transactions
GROUP BY customer_id,mn

)

select mn,count(customer_id) from cte
where deposit > 1 and (purchase <> 0 or withdrawal <> 0)
group by mn;
-- What is the closing balance for each customer at the end of the month?
with cte as (SELECT 
    MONTH(txn_date) mn,
    monthname(txn_date) mn_name,
    customer_id,
    SUM(CASE
...
FROM
    customer_transactions
GROUP BY customer_id , mn,mn_name)

select customer_id,mn_name,SUM(amount)OVER(PARTITION BY customer_id ORDER BY mn ROWS BETWEEN
   			   UNBOUNDED PRECEDING AND CURRENT ROW) AS closing_balance from cte
order by month(mn),customer_id;
...
SELECT 100 * COUNT(DISTINCT customer_id)/ (SELECT COUNT(DISTINCT customer_id) FROM customer_transactions) AS percentage_cutomer
...
-- To test out a few different hypotheses - the Data Bank team wants to run an experiment where different groups of customers would be allocated data using 3 different  
-- options:
...
select customer_id,txn_date,txn_type,sum(case when txn_type = 'deposit' then txn_amount else -txn_amount end) over(partition by customer_id order by txn_date rows between unbounded preceding and current row) running_total from customer_transactions;
-- customer balance at the end of each month
select customer_id,monthname(txn_date) mn,sum(case when txn_type = 'deposit' then txn_amount else -txn_amount end) total from customer_transactions
group by customer_id,mn order by customer_id;
-- minimum, average and maximum values of the running balance for each customer
with r_tot as (select customer_id,txn_date,txn_type,sum(case when txn_type = 'deposit' then txn_amount else -txn_amount end) over(partition by customer_id order by txn_date rows between unbounded preceding and current row) running_total from customer_transactions)
select customer_id,min(running_total),avg(running_total),max(running_total) from r_tot
